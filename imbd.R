...
setwd("H:\\RStudio\\assignment\\assignments1\\Text Mining")
...
x1 <- tm_map(x1, removeWords, c("comic","beefcake","literally","pointlesswhat","theater")) 
...
x1 <- tm_map(x1, removeWords, c("comic","beefcake","literally"))
x1 <- tm_map(x1, stripWhitespace)
tdm <- TermDocumentMatrix(x1)
tdm <- as.matrix(tdm)
